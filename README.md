...
9. **Use-case exceptions** — `claim.user` / `demo123`, open `CLM-2026-08391` (Layla Hassan). On a failed check, **Bypass**, add a comment, submit. Sign out. `claim.head` / `demo123` → same claim → **Approve**. That use-case’s score and weightage drop out of the stage score. Resolve still edits fields; Reject still waives a false positive.
