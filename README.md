...
1. **Queue** — every persona sees one Claims list (no Sort, no My claims / All claims). Filter by date range (7 / 30 / 60 / 90 / 180 / 365 days, or start and end dates) and Claim Stage. Claim Head / Admin / FIU: expand the chevron to open versions newest first; click a version for a change table plus claim details and use-case scores.
...
10. **Report** — `claim.head` / `demo123` → Report. Date range controls match the other filters. Use-case fail ranking is a table (rank, use-case, stage, failed claims, fail rate). Claim versions is one row per claim.
